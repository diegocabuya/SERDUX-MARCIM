...
delta = [0.25,1];%attack duration

Gamma = 3/((1/(eta))+ 1/(upsilon) + 1/(zeta)); %impact reduction controls
IRC = Gamma*100; %Impact Reduction controls:

X = zeros(length(delta), (tf*100)+1, 6);
for i = 1:length(delta)
    parameters = [lambda, a,m ,beta0, mu,L,psi,  eta,upsilon,delta(i),Gamma,iota*100,IRC];
...
legend({'(S) δ = 0.25', '(E) δ = 0.25', '(R) δ = 0.25', '(D) δ = 0.25', '(U) δ = 0.25', '(X) δ = 0.25', ...
        '(S) δ = 1', '(E) δ = 1', '(R) δ = 1', '(D) δ = 1', '(U) δ = 1', '(X) δ = 1'}, ...
        'Location', 'southoutside', 'NumColumns', 6, 'FontSize', 8,'Orientation','horizontal');

set(d,'Units','Inches');
pos = get(d,'Position');
set(d,'PaperPositionMode','Auto','PaperUnits','Inches','PaperSize',[pos(3), pos(4)])
print(d,'deltaX_test','-dpdf','-r0')
